...
int usb_gadget_register_driver(struct usb_gadget_driver *driver)
{
	...
	//printk("bind to %s --> %d", driver->driver.name, retval);
	if (udc_controller->transceiver) {
		...
		if(!strcmp(driver->driver.name, "g_file_storage"))
		{
			...
			LOG("Debug:bind udc driver to g_file_storage ok, set usb charge current to 240mA.\n");
			bind_flag = 1;
		}
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	{
		pmic_set_chg_current(0x4); //400mA
		LOG("Debug:unbind g_file_storage driver, set usb usb charge current to 400mA.\n");
		bind_flag = 0;

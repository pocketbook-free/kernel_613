...
#define DOWN_KEY			169
#define UP_KEY				42
#define BACKWARD_KEY	    46
#define FORWARD_KEY			41
...
{
	global_disable = lock;
	log("### %s:%d ###\n", __func__, global_disable);
	return 0;
}

/* sometimes power key event lost when exiting sleep mode,
 * (two up events was sent that was not recognized as keypress if keypress time is short)
 */
	...
	case GPIOKEY_STATE_PRESSED :
		if(state)
		{
			if (bdata->pressed_time >= button->longpress_time)
			{
				...
				log("%s:key_code=0x%x, key_state=%d\n", __func__, button->code, state);
				input_sync(input);
				bdata->pressed_time = 0;
			}
			else
...
static irqreturn_t gpio_keys_isr(int irq, void *dev_id)
{
	...
	if(!global_disable)
	{
		...
		//printk("button->debounce_interval:%x\n", button->debounce_interval);
		if (button->debounce_interval)
		{
			if (!gpio_get_value(button->gpio))
			{
				state = gpio_get_value(CONF_KEY) << 0;
				state |= gpio_get_value(LEFT_KEY) << 1;
				state |= gpio_get_value(RIGHT_KEY) << 2;
				state |= gpio_get_value(DOWN_KEY) << 3;
				state |= gpio_get_value(UP_KEY) << 4; 
				state |= gpio_get_value(BACKWARD_KEY) << 5;
				state |= gpio_get_value(FORWARD_KEY) << 6;
				log("get key value is 0x%x\n", state);
					 ...
					 struct gpio_keys_button *button)
{
	...
	//printk("%s: get irq number for GPIO %d, irq %d\n", __func__, button->gpio, irq);
	if (irq < 0) {
		error = irq;
...
/*
 *
 * key lock == key disable
 * key unlock == key enable
 * if key lock by user, enter sleep mode, set key can't resume system except power key, usb, sd card.
 * if key unlock, enter sleep mode, set any key could resume system.
 *
 */
#ifdef CONFIG_PM
static int gpio_keys_suspend(struct device *dev)
{
	...
	log("@@@ %s:get state:%d, keylock=%d @@@@\n", __func__, state, global_disable);
	if(state == PM_SUSPEND_STANDBY)
	{
		if (device_may_wakeup(&pdev->dev)) {
			for (i = 0; i < pdata->nbuttons; i++) {
				struct gpio_keys_button *button = &pdata->buttons[i];
				if (!global_disable && button->wakeup) {
					int irq = gpio_to_irq(button->gpio);
					enable_irq_wake(irq);
					log("%s:enable_irq_wake.\n", __func__);
				}
			}
		}
...
static int gpio_keys_resume(struct device *dev)
{
	...
	if(state == PM_SUSPEND_STANDBY)
	{
		...
		{
			struct gpio_keys_button *button = &pdata->buttons[i];
			if (!global_disable && button->wakeup && device_may_wakeup(&pdev->dev)) {
				int irq = gpio_to_irq(button->gpio);
				disable_irq_wake(irq);
				log("%s:disable_irq_wake.\n", __func__);
			}
		gpio_keys_report_event(&ddata->data[i]);
		}
	}
...
static int gpiokey_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	{
		case CM_KEYSTATE:
			keystate |= gpio_get_value(CONF_KEY) << 0;
			keystate |= gpio_get_value(LEFT_KEY) << 1;
			keystate |= gpio_get_value(RIGHT_KEY) << 2;
			keystate |= gpio_get_value(DOWN_KEY) << 3;
			keystate |= gpio_get_value(UP_KEY) << 4;
			keystate |= gpio_get_value(BACKWARD_KEY) << 5;
			keystate |= gpio_get_value(FORWARD_KEY) << 6;
			put_user(keystate, (int *)arg);
			log("%s:the value of keystate : %d\n", __func__, keystate);
			break;
		default : 
			break;

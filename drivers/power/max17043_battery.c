...
//	struct max17040_chip *chip = i2c_get_clientdata(client);
	...
	dev_dbg(chip->battery.dev, "%s(): entered\n", __func__);
...
//	struct max17040_chip *chip = i2c_get_clientdata(client);
	...
	rsoc_old = rsoc;
	rsoc = ((rsoc >> 8) & 0xff) / 2;
	dev_dbg(chip->battery.dev, "rsoc_old: %d, rsoc: %d\n", rsoc_old, rsoc);
...
#if 1
	// setting threshhold value for 0xC - 20% (???)
	if (rsoc > 10) {
	...
	// setting threshhold value for 0xC - 10% (???)
	if (rsoc < 10 && rsoc > 5) {
		max17044_read(0x0C, &value, client);
		value &= 0xffe0;
		value |= 0x0016;
	...
	// setting threshhold value for 0xC - 6% (???)
	if (rsoc < 5) {
		max17044_read(0x0C, &value, client);
		value &= 0xffe0;
		value |= 0x001a;
	...
	hw_set_batt(rsoc);
	dev_dbg(chip->battery.dev, "Battery capacity is %d\n", rsoc);
}
...
static int max17044_write(u8 reg, int wt_value, struct i2c_client *client)
{
	int ret;
...
static int max17044_read(u8 reg, int *rt_value, struct i2c_client *client)
{
	int ret;
...
static int max17044_write_code(u8 reg,int wt_value, struct i2c_client *client)
{
...
static int max17044_read_code(u8 reg, int *rt_value, struct i2c_client *client)
{
	struct i2c_msg msg[1];
...
{
	int ret;
#if 1	// import new capacity table	0605 EP8
/*****************set 64 byte mode ******************************/ 
	char data0[] = {0xAF,0x00,0xB7,0x60,0xB9,0xC0,0xBA,0xF0,
			  0xBC,0x30,0xBC,0x70,0xBC,0xC0,0xBD,0x10};

	char data1[] = {0xBD,0x60,0xBE,0x30,0xBF,0x00,0xC0,0xA0,
			  0xC4,0x40,0xC7,0xF0,0xCB,0xC0,0xCF,0xA0};

	char data2[] = {0x03,0x00,0x1E,0x00,0x1E,0x00,0x1C,0x80,
			  0x96,0x90,0x78,0x70,0x78,0x70,0x78,0x70};

	char data3[] = {0x2C,0x40,0x2C,0x40,0x2C,0x40,0x1B,0x40,
			  0x1A,0xC0,0x12,0x30,0x11,0xF0,0x11,0xF0};
...
static void max17044_burn(struct i2c_client *client)
{
	...
	max17044_write_rccomp(client, 0x4200);          //4.write rcomp to a maximum value of 0xff00
...
static void max17043_lbirq_work(struct work_struct *work)
{
	...
	chip = container_of(work, struct max17040_chip, lwork);
			...
			const struct i2c_device_id *id)
{
	...
	INIT_WORK(&chip->lwork, max17043_lbirq_work);
		...
		pm_message_t state)
{
	...
	cancel_delayed_work(&chip->work);
	enable_irq_wake(gpio_to_irq(103)); // GP4_7
...
static int max17040_resume(struct i2c_client *client)
{
	...
	// why disable IRQ?
	disable_irq_wake(gpio_to_irq(103)); // GP4_7
	schedule_delayed_work(&chip->work, msecs_to_jiffies(MAX17040_DELAY));

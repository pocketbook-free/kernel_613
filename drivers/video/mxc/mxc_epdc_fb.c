...
{
	u32 reg_val = 0;
	/*	for EP8 do not support A2 mode by hugo */
	if(4 == waveform_mode)
		waveform_mode = 1;
...
#ifdef VCOM_TEST_ONLY
	printk(" Keep vcom and power status.\n");
#else 
	//printk(" Disable vcom and power.\n");
				 ...
				 struct fb_info *info)
{
	...
	/*	for EP8 rotate the EPD add by hugo	*/
	switch (var->rotate){
	case 0:
		var->rotate = 2;
		break;
...
/*for test display performance*/
/*	if(upd_data->waveform_mode == 2)
			upd_data->waveform_mode = 3;*/
...
//	fb_data->cur_mode = &fb_data->pdata->epdc_mode[1]; //E60_V220
	fb_data->cur_mode = &fb_data->pdata->epdc_mode[0]; //E60_V110 for gerneration 1 panel

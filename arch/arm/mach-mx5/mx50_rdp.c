...
#define CHRG_Indicator (5*32 + 11) /* GPIO_6_11*/   //EP8
...
/* add patch for cpu crash issue from freescale 20120607 */
...
static unsigned int sdhc_get_card_det_status(struct device *dev)
{
	...
	if (to_platform_device(dev)->id == 0) {
		...
		if(ret == 0) {
			...
			//mxc_iomux_v3_setup_pad(MX50_PAD_SD1_EN__GPIO_4_2);//EP7
			mxc_iomux_v3_setup_pad(MX50_PAD_DISP_D8__GPIO_2_8);//EP8
			gpio_request(SD1_EN, "sdhc1-en");
...
{
	printk("%s\n", __func__); //add by hugo for debug boot info
	/* SD card detect irqs */
